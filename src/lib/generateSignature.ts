// Mark renders at a fixed 30px height; width is derived from each asset's
// own native aspect ratio (symbol 200x200, wordmark 572.33x80) so neither
// is stretched. The embedded PNGs are rasterized well above this display
// size, so downscaling here stays crisp on retina.
const MARK_H      = 30
const SYMBOL_W    = 30
const WORDMARK_W  = 215

// -2% letter-spacing (-0.02em), expressed as an absolute px value so it's
// consistent across email clients regardless of how they resolve em units.
const FONT_BASE = 'font-family:Arial,Helvetica,sans-serif;font-size:15px;letter-spacing:-0.3px'
...
${wrapWithMargin(signatureTable(fields, colors.primary, colors.secondary, colors.divider, signatureMarks))}
</body></html>`
}
...
<tr><td style="padding:0;">
  ...
  <tr>
    <td style="width:50%;vertical-align:bottom;">
      <img src="${m.symbol}" width="${SYMBOL_W}" height="${MARK_H}" alt="" border="0"
           style="display:block;border:0;width:${SYMBOL_W}px;height:${MARK_H}px;">
    </td>
    <td style="width:50%;text-align:right;vertical-align:bottom;">
      <img src="${m.wordmark}" width="${WORDMARK_W}" height="${MARK_H}" alt="" border="0"
...
<tr><td style="padding:0;">
  ...
  <tr>
    <td style="width:50%;vertical-align:bottom;">
      <img src="${marks.symbol}" width="${SYMBOL_W}" height="${MARK_H}" alt="" border="0"
           style="display:block;border:0;width:${SYMBOL_W}px;height:${MARK_H}px;">
    </td>
    <td style="width:50%;text-align:right;vertical-align:bottom;">
      <img src="${marks.wordmark}" width="${WORDMARK_W}" height="${MARK_H}" alt="" border="0"
           style="display:block;border:0;width:${WORDMARK_W}px;height:${MARK_H}px;margin-left:auto;">
    </td>
  </tr>
  </table>
